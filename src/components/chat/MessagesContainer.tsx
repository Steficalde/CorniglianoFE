...
export default function MessagesContainer() {
  ...
  async function chat() {
    ...
    try {
      ...
      // @ts-ignore
      console.log(e.error)
      setMessages((messages) => [ ...messages,"problema di connessione, riprovare" ])
    } finally {
  ...
  return (
      ...
      >
        <div id="container-messages" className="overflow-auto flex flex-col mt-3 p-2">
          {
            messages.map((message, index) => {
              issuer = !issuer
              return <Message key={index} issuer={issuer} >{message}</Message>
            })
          }
          {isLoading && <Message issuer={false}>
            <ThreeDots
                height="30"
                width="30"
                radius="2"
        ...
        <div id="container-sender" className=" h-[60px] w-full flex justify-center items-center mb-2">
          <form
              onSubmit={(e) => {
                e.preventDefault()
              }}
